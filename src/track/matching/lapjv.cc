/*
 * @Author: lwn
 * @Date: 2024-04-07 13:42:23
 * @LastEditors: lwn
 * @LastEditTime: 2024-04-07 14:32:19
 * @FilePath: /FalconEye/src/track/matching/lapjv.cc
 * @Description:
 */
#include <stdio.h>
#include <stdlib.h>
 ...
 */
int
_ccrrt_dense(const int n, float *cost[], int *free_rows, int *x, int *y, float *v)
{
    int n_free_rows;
    bool *unique;
 ...
 */
int
_carr_dense(const int n, float *cost[], const int n_free_rows, int *free_rows, int *x, int *y, float *v)
{
    int current = 0;
    int new_free_rows = 0;
    int rr_cnt = 0;
 ...
 */
int
_find_dense(const int n, int lo, float *d, int *cols, int *y)
{
    int hi = lo + 1;
...
int
_scan_dense(const int n, float *cost[], int *plo, int *phi, float *d, int *cols, int *pred, int *y, float *v)
{
    int lo = *plo;
    int hi = *phi;
    float h, cred_ij;
 ...
 */
int
find_path_dense(const int n, float *cost[], const int start_i, int *y, float *v, int *pred)
{
    int lo = 0, hi = 0;
    int final_j = -1;
    int n_ready = 0;
 ...
 */
int
_ca_dense(const int n, float *cost[], const int n_free_rows, int *free_rows, int *x, int *y, float *v)
{
    int *pred;
...
lapjv_internal(const cv::Mat &cost, const bool extend_cost, const float cost_limit, int *x, int *y)
{
    ...
    {
        throw std::invalid_argument("Square cost array expected. If cost is intentionally non-square, pass "
                                    "extend_cost=True.");
    }
